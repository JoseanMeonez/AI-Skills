...
- **Tool mapping** — added `references/copilot-tools.md` with the full the AI CLI (e.g., Gemini CLI / Claude Code) to Copilot CLI tool equivalence table
...
- **codex-tools** — added named agent dispatch mapping documenting how to translate the AI CLI (e.g., Gemini CLI / Claude Code)'s named agent types to Codex's `spawn_agent` with worker roles (PR #647 by @arittr)
...
- **Generic agent language** — brainstorm companion waiting page now says "the agent" instead of "the AI assistant".
...
- Gemini CLI tool mapping reference (`skills/using-superpowers/references/gemini-tools.md`) — translates the AI CLI (e.g., Gemini CLI / Claude Code) tool names (Read, Write, Edit, Bash, etc.) to Gemini CLI equivalents (read_file, write_file, replace, etc.)
...
- Per-platform launch instructions in visual-companion.md: the AI CLI (e.g., Gemini CLI / Claude Code) (default mode), Codex (auto-foreground via `CODEX_CI`), Gemini CLI (`--foreground` with `is_background`), and fallback for other environments
...
- Verified on Windows 11 (NT 10.0.26200.0) with the AI CLI (e.g., Gemini CLI / Claude Code) 2.1.72 and Git for Windows
...
- Tested with `the AI assistant --plugin-dir` and `the AI assistant-session-driver`: worker now correctly dispatches the reviewer
...
- Hook now detects whether it's running in the AI CLI (e.g., Gemini CLI / Claude Code) or another platform
- Emits `hookSpecificOutput` for the AI CLI (e.g., Gemini CLI / Claude Code), `additional_context` for others — prevents double context injection
...
- @karuturi — the AI CLI (e.g., Gemini CLI / Claude Code) official marketplace install instructions (PR #610)
...
Writing-plans no longer offers a choice between subagent-driven and executing-plans. On harnesses with subagent support (the AI CLI (e.g., Gemini CLI / Claude Code), Codex), subagent-driven-development is required. Executing-plans is reserved for harnesses without subagent capability, and now tells the user that Superpowers works better on a subagent-capable platform.
...
If AGENTS.md or AGENTS.md says "don't use TDD" and a skill says "always use TDD," the user's instructions win.
...
- Platform Adaptation pointer added so non-the AI CLI platforms can find tool equivalents
- Plan headers now address "agentic workers" instead of "the AI assistant" specifically
...
the AI CLI (e.g., Gemini CLI / Claude Code)'s `.sh` auto-detection on Windows was prepending `bash` to the hook command, breaking execution. The fix:
...
This fix should dramatically improve superpowers skills compliance and should reduce the chances of the AI assistant entering its native plan mode unintentionally.
...
Added an `EnterPlanMode` intercept to the skill flow graph. When the model is about to enter the AI assistant's native plan mode, it checks whether brainstorming has happened and routes through the brainstorming skill instead. Plan mode is never entered.
...
**Windows: Fixed the AI CLI (e.g., Gemini CLI / Claude Code) 2.1.x hook execution (#331)**

the AI CLI (e.g., Gemini CLI / Claude Code) 2.1.x changed how hooks execute on Windows: it now auto-detects `.sh` files in commands and prepends `bash`. This broke the polyglot wrapper pattern because `bash "run-hook.cmd" session-start.sh` tries to execute the `.cmd` file as a bash script.

Fix: hooks.json now calls session-start.sh directly. the AI CLI (e.g., Gemini CLI / Claude Code) 2.1.x handles the bash invocation automatically. Also added .gitattributes to enforce LF line endings for shell scripts (fixes CRLF issues on Windows checkout).
...
Improved documentation of how Codex tools map to the AI CLI (e.g., Gemini CLI / Claude Code) equivalents for subagent workflows.
...
**the AI CLI (e.g., Gemini CLI / Claude Code): Fixed Windows hook execution for the AI CLI (e.g., Gemini CLI / Claude Code) 2.1.x**
...
Addressed a failure mode where the AI assistant would skip invoking a skill even when the user explicitly requested it by name (e.g., "subagent-driven-development, please"). the AI assistant would think "I know what that means" and start working directly instead of loading the skill.

Changes:
- Updated "The Rule" to say "Invoke relevant or requested skills" instead of "Check for skills" - emphasizing active invocation over passive checking
- Added "BEFORE any response or action" - the original wording only mentioned "response" but the AI assistant would sometimes take action without responding first
...
New test suite in `tests/explicit-skill-requests/` that verifies the AI assistant correctly invokes skills when users request them by name. Includes single-turn and multi-turn test scenarios.
...
Added `disable-model-invocation: true` to all three slash commands (`/brainstorm`, `/execute-plan`, `/write-plan`). the AI assistant can no longer invoke these commands via the Skill tool—they're restricted to manual user invocation only.

The underlying skills (`superpowers:brainstorming`, `superpowers:executing-plans`, `superpowers:writing-plans`) remain available for the AI assistant to invoke autonomously. This change prevents confusion when the AI assistant would invoke a command that just redirects to a skill anyway.

## v4.0.1 (2025-12-23)

### Fixes

**Clarified how to access skills in the AI CLI (e.g., Gemini CLI / Claude Code)**

Fixed a confusing pattern where the AI assistant would invoke a skill via the Skill tool, then try to Read the skill file separately. The `using-superpowers` skill now explicitly states that the Skill tool loads skill content directly—no need to read files.
...
`tests/claude-code/` - Integration tests using `the AI assistant -p` for headless testing. Verifies skill usage via session transcript (JSONL) analysis. Includes `analyze-token-usage.py` for cost tracking.
...
**The Description Trap** (documented in `writing-skills`): Discovered that skill descriptions override flowchart content when descriptions contain workflow summaries. the AI assistant follows the short description instead of reading the detailed flowchart. Fix: descriptions must be trigger-only ("Use when X") with no process details.
...
- **docs/testing.md** - Guide to testing skills with the AI CLI (e.g., Gemini CLI / Claude Code) integration tests
...
- Clarified `writing-skills` guidance so it points to the correct agent-specific personal skill directories (`~/.claude/skills` for the AI CLI (e.g., Gemini CLI / Claude Code), `~/.codex/skills` for Codex).
...
**Key differences from the AI CLI (e.g., Gemini CLI / Claude Code) integration:**
...
The Skill tool is the proper mechanism for invoking skills in the AI CLI (e.g., Gemini CLI / Claude Code). This update corrects the bootstrap instructions to guide agents toward the correct tool.
...
- **Fixed command syntax in README** (#44) - Updated all command references to use correct namespaced syntax (`/superpowers:brainstorm` instead of `/brainstorm`). Plugin-provided commands are automatically namespaced by the AI CLI (e.g., Gemini CLI / Claude Code) to avoid conflicts between plugins.
...
- Added workflow checklists the AI assistant can copy and track
...
- **Fixed session-start hook execution in plugin context** (#8, PR #9) - The hook was failing silently with "Plugin hook error" preventing skills context from loading. Fixed by:
  - Using `${BASH_SOURCE[0]:-$0}` fallback when BASH_SOURCE is unbound in the AI CLI (e.g., Gemini CLI / Claude Code)'s execution context
...
Beyond infrastructure, this release adds nine new skills focused on problem-solving, research, and architecture. We rewrote the core **using-skills** documentation with imperative tone and clearer structure, making it easier for the AI assistant to understand when and how to use skills. **find-skills** now outputs paths you can paste directly into the Read tool, eliminating friction in the skills discovery workflow.
...
- Improved CSO (the AI assistant Search Optimization) guidance
...
```bash
# In the AI CLI (e.g., Gemini CLI / Claude Code)
