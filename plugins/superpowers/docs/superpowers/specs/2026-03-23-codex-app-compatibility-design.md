...
Make superpowers skills work in the Codex App's sandboxed worktree environment without breaking existing the AI CLI (e.g., Gemini CLI / Claude Code) or Codex CLI behavior.
...
- **IN_LINKED_WORKTREE:** `GIT_DIR != GIT_COMMON` — the agent is in a worktree created by something else (Codex App, the AI CLI (e.g., Gemini CLI / Claude Code) Agent tool, previous skill run, or the user)
...
|---|---|---|---|
| No | No | the AI CLI (e.g., Gemini CLI / Claude Code) / Codex CLI / normal git | Full skill behavior (unchanged) |
...
- The 4-option finishing menu — preserved exactly for the AI CLI (e.g., Gemini CLI / Claude Code) and Codex CLI
...
### Automated (run in the AI CLI (e.g., Gemini CLI / Claude Code) after implementation)
...
- Existing the AI CLI (e.g., Gemini CLI / Claude Code) skill-triggering tests still pass
- Existing subagent-driven-development integration tests still pass
- Normal the AI CLI (e.g., Gemini CLI / Claude Code) session: full worktree creation + 4-option finishing still works
