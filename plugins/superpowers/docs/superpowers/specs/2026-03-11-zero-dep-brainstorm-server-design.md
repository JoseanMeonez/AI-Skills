...
1. **`GET /`** — Serve newest `.html` from screen directory by mtime. Detect full documents vs fragments, wrap fragments in frame template, inject helper.js. Return `text/html`. When no `.html` files exist, serve a hardcoded waiting page ("Waiting for the AI assistant to push a screen...") with helper.js injected.
...
- Shell scripts require bash (Git Bash on Windows, which is required for the AI CLI (e.g., Gemini CLI / Claude Code))
