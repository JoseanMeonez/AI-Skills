...
During visual brainstorming, the AI assistant runs `wait-for-feedback.sh` as a background task and blocks on `TaskOutput(block=true, timeout=600s)`. This seizes the TUI entirely — the user cannot type to the AI assistant while visual brainstorming is running. The browser becomes the only input channel.

the AI CLI (e.g., Gemini CLI / Claude Code)'s execution model is turn-based. There is no way for the AI assistant to listen on two channels simultaneously within a single turn. The blocking `TaskOutput` pattern was the wrong primitive — it simulates event-driven behavior the platform doesn't support.
...
**Terminal = conversation channel.** Always unblocked, always available. The user talks to the AI assistant here.

### The Loop

1. the AI assistant writes an HTML file to the session directory
2. Server detects it via chokidar, pushes WebSocket reload to the browser (unchanged)
3. the AI assistant ends its turn — tells the user to check the browser and respond in the terminal
4. User looks at browser, optionally clicks to select an option, then types feedback in the terminal
5. On the next turn, the AI assistant reads `$SCREEN_DIR/.events` for the browser interaction stream (clicks, selections), merges with the terminal text
...
Deleted entirely. Its purpose was to bridge "server logs events to stdout" and "the AI assistant needs to receive those events." The `.events` file replaces this — the server writes user interaction events directly, and the AI assistant reads them with whatever file-reading mechanism the platform provides.

### Key Addition: `.events` File (Per-Screen Event Stream)

The server writes all user interaction events to `$SCREEN_DIR/.events`, one JSON object per line. This gives the AI assistant the full interaction stream for the current screen — not just the final selection, but the user's exploration path (clicked A, then B, settled on C).
...
- If the file doesn't exist when the AI assistant reads it, no browser interaction occurred — the AI assistant uses only the terminal text.
...
- the AI assistant can read the full stream to understand the user's exploration pattern, or just look at the last `choice` event for the final selection.
...
The current regex anchors on `<div class="feedback-footer">`, which is being removed. Replace with a comment placeholder: remove the existing default content inside `#the AI assistant-content` (the `<h2>Visual Brainstorming</h2>` and subtitle paragraph) and replace with a single `<!-- CONTENT -->` marker. Content injection becomes `frameTemplate.replace('<!-- CONTENT -->', content)`. Simpler and won't break if template formatting changes.
...
**Add:**
- `<!-- CONTENT -->` placeholder inside `#the AI assistant-content`, replacing the default text
...
- `.main` wrapper and `#the AI assistant-content` container
...
**Remove:**
- `sendToClaude()` function and the "Sent to the AI assistant" page takeover
...
- "User Feedback Format" section describing `send-to-the AI assistant` JSON
...
The server code (`index.js`, `helper.js`, `frame-template.html`) is fully platform-agnostic — pure Node.js and browser JavaScript. No the AI CLI (e.g., Gemini CLI / Claude Code)-specific references. Already proven to work on Codex via background terminal interaction.

The skill instructions (`visual-companion.md`) are the platform-adaptive layer. Each platform's the AI assistant uses its own tools to start the server, read `.events`, etc. The non-blocking model works naturally across platforms since it doesn't depend on any platform-specific blocking primitive.
...
- **Cross-platform** — same server code works on the AI CLI (e.g., Gemini CLI / Claude Code), Codex, and any future platform
...
- **Immediate response on browser Send** — the old system had the AI assistant respond the moment the user clicked Send. Now there's a gap while the user switches to the terminal. In practice this is seconds, and the user gets to add context in their terminal message.
