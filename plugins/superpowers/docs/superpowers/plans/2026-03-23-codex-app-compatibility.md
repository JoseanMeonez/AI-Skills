...
**If `GIT_DIR` differs from `GIT_COMMON`:** You are already inside a linked worktree (created by the Codex App, the AI CLI (e.g., Gemini CLI / Claude Code)'s Agent tool, a previous skill run, or the user). Do NOT create another worktree. Instead:
...
Note: these tests require the AI CLI (e.g., Gemini CLI / Claude Code) with `--dangerously-skip-permissions`. If not available, document that regression tests should be run manually.
